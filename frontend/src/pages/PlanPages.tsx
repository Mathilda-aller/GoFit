...
import { useActionCards } from "../api";
import { AppShell, MuscleMap, PageHeader } from "../components";
import { actionCards, recommendations } from "../mocks/data";
...
export function PlansPage() {
  ...
  const start = (planId: string) => {
    const sessionId = startSession(planId);
    ...
    <AppShell>
      <main className="page">
        ...
        <div className="hero-panel">
          ...
          <button className="button button--primary" onClick={create}><Plus size={18} />新建练单</button>
        </div>
        ...
        <section className="section">
          ...
          {plans.length === 0 ? (
            <div className="surface empty-state"><div><Dumbbell size={28} /><h3>还没有保存的练单</h3><p className="muted">新建一份，把想练的视频动作卡放进去。</p><button className="button button--primary" onClick={create}>新建练单</button></div></div>
          ) : (
            plans.map((plan) => (
              <article className="surface plan-card" key={plan.id}>
                ...
                <div className="chip-row">
                  {plan.cardIds.slice(0, 3).map((cardId) => <span className="chip" key={cardId}>{actionCards[cardId]?.actionCard.actionName}</span>)}
                </div>
                <div className="button-row" style={{ marginTop: 16 }}>
                  <Link className="button button--neutral" to={`/plans/${plan.id}`}>查看练单</Link>
                  <button className="button button--primary" disabled={!plan.cardIds.length} onClick={() => start(plan.id)}><Play size={17} />开始</button>
                </div>
              </article>
            ))
          )}
...
function DeletePlanDialog({ planName, onDelete }: { planName: string; onDelete: () => void }) {
  return (
    <Dialog.Root>
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="dialog-content" aria-describedby="delete-plan-description">
          ...
          <p className="muted" id="delete-plan-description">练单和相关训练记录会一并删除，此操作无法撤销。</p>
          <div className="button-row" style={{ marginTop: 20 }}>
            <Dialog.Close asChild><button className="button button--neutral">取消</button></Dialog.Close>
            <Dialog.Close asChild><button className="button button--danger" onClick={onDelete}><Trash2 size={17} />确认删除</button></Dialog.Close>
          </div>
        </Dialog.Content>
      </Dialog.Portal>
    </Dialog.Root>
...
export function PlanDetailPage() {
  ...
  const cards = plan.cardIds.map((id) => actionCards[id]).filter(Boolean);
  const primary = [...new Set(cards.flatMap((response) => response.actionCard.primaryMuscles))];
  const secondary = [...new Set(cards.flatMap((response) => response.actionCard.secondaryMuscles))];
  const candidates = recommendations.filter(({ cardId }) => !plan.cardIds.includes(cardId) && !uninterested.includes(cardId));
  const start = () => {
      ...
      <main className="page page--immersive">
        ...
        <div className="plan-name-field">
          ...
          <input
            ...
            onBlur={(event) => renamePlan(plan.id, event.target.value)}
          />
          <p className="muted">{plan.cardIds.length} 个动作 · 已使用 {plan.useCount} 次</p>
        </div>

        <details className="compact-disclosure section">
          <summary>
            <span><small>肌群覆盖</small><strong>{primary.length ? primary.join("、") : "等待添加动作"}</strong></span>
            <ChevronRight size={18} />
          </summary>
          <div className="compact-disclosure__body"><MuscleMap primary={primary.length ? primary : ["等待添加动作"]} secondary={secondary} /></div>
        </details>
        ...
        <section className="section">
          ...
          <div className="surface" style={{ padding: "0 15px" }}>
            {plan.cardIds.length ? plan.cardIds.map((cardId, index) => {
              const response = actionCards[cardId];
              ...
              return (
                <div className="plan-item" key={cardId}>
                  ...
                  <div className="plan-item__actions">
                    <button className="mini-button" aria-label="上移" disabled={index === 0} onClick={() => moveCard(plan.id, cardId, -1)}><ArrowUp size={16} /></button>
                    <button className="mini-button" aria-label="下移" disabled={index === plan.cardIds.length - 1} onClick={() => moveCard(plan.id, cardId, 1)}><ArrowDown size={16} /></button>
                    <button className="mini-button" aria-label="移除" onClick={() => removeCard(plan.id, cardId)}><Trash2 size={16} /></button>
                  </div>
                </div>
              );
            }) : (
        ...
        {plan.cardIds.length ? (
          <details className="compact-disclosure section plan-assistant" open>
            ...
            <div className="compact-disclosure__body">
              {candidates.length ? candidates.map(({ cardId, reason }) => {
                const card = actionCards[cardId].actionCard;
                return (
                  <div className="recommendation" key={cardId}>
                    ...
                    <div className="button-row">
                      ...
                      <button className="button button--soft" onClick={() => addCard(plan.id, cardId)}><Plus size={16} />加入</button>
                    </div>
                  </div>
                );
        ...
        <div className="sticky-actions">
          <button className="button button--primary button--wide" onClick={start} disabled={!plan.cardIds.length}><Play size={19} />开始训练</button>
        </div>
      </main>
    </AppShell>
  );
  ...
  return (
    <AppShell navigation={false}>
      <main className="page page--immersive">
        ...
        <div className="chip-row" style={{ marginTop: 18 }}><span className="chip chip--accent">肩部</span><span className="chip">哑铃</span><span className="chip">全部</span></div>
          ...
          {cards.map((response) => {
            ...
            return (
              <div className="list-card" key={card.sourceVideo.videoId}>
                ...
                <button className={`button ${added ? "button--neutral" : "button--soft"}`} style={{ minHeight: 40, padding: "0 12px" }} disabled={added} onClick={() => add(card.sourceVideo.videoId)}>
                  {added ? "已加入" : "加入"}
                </button>
              </div>
            );
