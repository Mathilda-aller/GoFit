...
function shell(content) {
  ...
  return `<div class="app-shell ${sourceMode ? "source-shell" : ""}">
    <aside class="sidebar">
      ...
      <div class="sidebar-note"><p>本地演示空间</p><div class="status-row"><span class="status-dot"></span>AI mock 已连接</div></div>
    </aside>
    <main class="main">
...
function renderExperience() {
  ...
  return `<div class="experience-header"><div><div class="eyebrow">P08 · PEER EXPERIENCE</div><h1>练友经验。</h1><p class="lede">${card.name} · ${question}</p></div><div class="experience-source">AI 整理自 ${card.videoCount} 条同动作视频下的 ${card.experienceCount} 条公开评论</div></div><div class="question-tabs"><button class="question-tab ${question === "手臂更有感觉" ? "active" : ""}">手臂更有感觉</button><button class="question-tab ${question === "目标肌群无感" ? "active" : ""}">目标肌群无感</button><button class="question-tab ${question === "重量或动作控制" ? "active" : ""}">重量或动作控制</button></div><div class="panel" style="padding:19px;margin-bottom:18px"><div class="eyebrow">当前视频是否提到</div><p style="margin:8px 0 0;font-size:12px;color:var(--muted)">当前视频在 <b style="color:var(--ink)">00:24</b> 提到：不要为了抬高而耸肩。下面的解决方法来自其他同动作视频的公开评论。</p></div><div class="experience-list">${experienceGroups.map((group, index) => `<article class="experience-card"><div class="experience-card-head"><div><h3>${group.title}</h3><p>${group.summary}</p></div><span class="mention">${group.count} 条 · ${group.videos} 视频</span></div><div style="display:flex;justify-content:space-between;align-items:center;gap:12px"><span class="muted">${group.split}</span><button class="btn btn-secondary" data-action="toggle-comments" data-index="${index}">查看依据</button></div><div class="comment-list" id="comments-${index}">${group.comments.map(comment => `<div class="comment"><strong>${comment[0]}</strong>${comment[1]}<div style="margin-top:6px;color:var(--green);font-size:10px">查看来源视频 · 对我有用</div></div>`).join("")}</div></article>`).join("")}</div><div style="display:flex;gap:9px;margin-top:19px"><button class="btn btn-primary" data-action="back-training">返回训练</button><button class="btn btn-secondary" data-action="mark-useful">${state.useful.length ? "已标记有用" : "这组经验对我有用"}</button></div>`;
}
...
function handleAction(event) {
  ...
  if (action === "open-experience") return route("experience", { experienceQuestion: "手臂更有感觉" });
  if (action === "new-plan") return showNewPlanModal();
  if (action === "open-plan") { state.selectedPlan = el.dataset.plan; return route("detail"); }
  ...
  if (action === "add-to-plan") { addCardToPlan(el.dataset.card, state.selectedPlan); return; }
...
function submitFeedback() {
  ...
  if (["OTHER_FELT", "NO_FEELING"].includes(state.selectedFeedback)) { saveFeedbackToHistory(); if (state.session) state.session.currentRecorded = true; state.experienceQuestion = state.selectedFeedback === "OTHER_FELT" ? `${state.selectedBody || "其他部位"}更有感觉` : "目标肌群无感"; saveState(); return route("experience"); }
...
function saveFeedbackToHistory() {
  const card = currentSessionCard(); state.history.unshift({ name: card.name, felt: feedbackLabel(state.selectedFeedback), date: "刚刚", color: "neutral" });
  if (state.session) state.session.completed.push({ cardId: card.id, feedback: state.selectedFeedback, feedbackLabel: feedbackLabel(state.selectedFeedback), body: state.selectedBody });
}
...
function addCardToPlan(cardId, planId) { const plan = getPlan(planId); if (!plan.cardIds.includes(cardId)) { plan.cardIds.push(cardId); plan.updated = "刚刚"; saveState(); render(); toast(`已加入「${plan.name}」`); } else toast("这张卡已经在这份练单里了"); }
function removeItem(index) { const plan = getPlan(); plan.cardIds.splice(index, 1); plan.updated = "刚刚"; saveState(); render(); toast("已从练单移除"); }
function moveItem(index, direction) { const plan = getPlan(); const target = direction === "up" ? index - 1 : index + 1; if (target < 0 || target >= plan.cardIds.length) return; [plan.cardIds[index], plan.cardIds[target]] = [plan.cardIds[target], plan.cardIds[index]]; plan.updated = "刚刚"; saveState(); render(); }
...
function handleModalAction(event) {
  ...
  if (action === "create-plan") { const name = document.getElementById("new-plan-name").value.trim() || "未命名练单"; const plan = { id: `plan-${Date.now()}`, name, cardIds: [], uses: 0, updated: "刚刚" }; state.plans.unshift(plan); state.selectedPlan = plan.id; closeModal(); saveState(); return route("detail"); }
  if (action === "choose-plan") { addCardToPlan(el.dataset.card, el.dataset.plan); closeModal(); state.selectedPlan = el.dataset.plan; return route("detail"); }
  if (action === "create-plan-with-card") { const plan = { id: `plan-${Date.now()}`, name: "未命名练单", cardIds: [el.dataset.card], uses: 0, updated: "刚刚" }; state.plans.unshift(plan); state.selectedPlan = plan.id; closeModal(); saveState(); return route("detail"); }
  if (action === "choose-card-for-plan") { addCardToPlan(el.dataset.card, state.selectedPlan); closeModal(); return; }
}
